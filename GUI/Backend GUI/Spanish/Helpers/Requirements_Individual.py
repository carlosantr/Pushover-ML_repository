        ...
        if variables[var] != "":#Verificar si contiene algo
            if var in ["Nx", "Ny"]:
                if (variables[var].get()).isdigit():#Verificar que sea un número entero positivo
                    ...
                    cumplimiento = f"{var} debe ser un número entero positivo."
                    break
            else:
                try:
                    variables_transformadas[var]=float(variables[var].get())
                except:
                    cumplimiento = f"{var} debe ser un número válido."  
                    break
            if variables_transformadas[var]<=0:
                cumplimiento = f"{var} debe ser un número mayor que 0."  
                break
        else:
            cumplimiento = f"No hay ningun valor ingresado para {var}" 
            break
...
def comprobacion_rangos(variables):
    ...
    for var in variables.keys():
        if (variables[var] < rangos[var][0])|(variables[var] > rangos[var][1]):
            ...
            cumplimiento = f"Fuera del rango permitido en la variable {var_txt} ({rangos[var]})"
            break
    return cumplimiento
...
def comprobacion_norma_general(variables):
    ...
    #C.21.3.5.1
    if variables["B"]<0.25:
        cumplimiento = "Incumple norma C.21.3.5.1 (NSR-10)\n(B >= 0.25m)"
    #C.9.5.2.1
    if (variables["H"]<LimH1)|(variables["H"]>LimH2):
        cumplimiento = "Incumple norma C.9.5.2.1 (NSR-10)\n(Lx/12 >= H >= Lx/16)"
    #C.10.4.1
    if variables["Lx"]/variables["B"]>50:
        cumplimiento = "Incumple norma C.10.4.1 (NSR-10)\n(Lx/B > 50)"
    #Altura/Base
    if (variables["B"]<LimB1)|(variables["B"]>LimB2):
        cumplimiento = "Incumple relación entre altura y base de vigas\n(H/1.4 >= B >= H/1.2)"
    #C.3.21.3.6.2.2
    OK, n_barras_C, num_barra_C, n_barras_V, num_barra_V = comprobacion_norma(variables, "Individual")
    if OK == 0:
        cumplimiento = "Incumple norma C.3.21.3.6.2.2 [SCWB]\n(Mnc >= 1.2Mnb)"
