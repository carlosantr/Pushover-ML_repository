        ...
        if archivo:  # Si el usuario selecciona una ubicación
            try:
                ...
                with ExcelWriter(archivo, engine='xlsxwriter') as writer:
                    ...
                    df_resultados_export.to_excel(writer, sheet_name="Resúmen de predicciones", index=True)  # Hoja 1
                    messagebox.showinfo("Exportación exitosa", f"Archivo guardado en: {archivo}")
            except Exception as e:
                messagebox.showerror("Error", f"Fallo en exportar el archivo:\n{str(e)}")
    boton_exportar = Button(res_frame,text="Exportar a Excel",command=exportar_a_excel,
