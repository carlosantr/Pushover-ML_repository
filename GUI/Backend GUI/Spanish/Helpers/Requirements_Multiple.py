...
def comprobacion_structure(data):
    ...
    if set(actual_cols) == set(expected_cols):
        ...
        cumplimiento = "Estructura incorrecta en los datos de entrada (véase el botón de '?'):\n"
        if missing_cols:
            cumplimiento += f"Columnas faltantes: ({', '.join(missing_cols)})\n"
        if extra_cols:
            cumplimiento += f"Columnas incorrectas: ({', '.join(extra_cols)})\n"
...
def comprobacion_formato(data):
    ...
    if not filas_vacias.empty:
        ...
        error_message += f"* Celdas vacías en: {', '.join(map(str, indices_vacios))}\n"
    ...
    for var in data.columns:
        if var in ["Nx", "Ny"]:  # Integer
            ...
            if not filas_no_enteros_positivos.empty:
                error_message += f"* {var} debe ser un número entero positivo: Vea filas {', '.join(map(str, [i + 1 for i in indices_no_enteros_positivos]))}\n"
            ...
            if not filas_no_floats.empty:
                error_message += f"* {var} contiene valores negativos o inválidos: Vea filas {', '.join(map(str, [i + 1 for i in indices_no_floats]))}\n"            
    if error_message != "":
...
def comprobacion_rangos(data, rangos):
    ...
    for var in data.columns:
        ...
        if not filas_fuera_rango.empty:
            error_message += f"* {var_txt} fuera del rango permitido ({rangos[var]}): Vea filas {', '.join(map(str, [i + 1 for i in indices_fuera_rango]))}\n"
    if error_message != "":
        cumplimiento = error_message
    return cumplimiento
...
def comprobacion_norma_general(data):
    ...
    for i in range(len(data)):
        ...
        if data.loc[i,"B"]<0.25:
            msg_i += "Incumple norma C.21.3.5.1 (NSR-10)\n(B >= 0.25m)\n"
        #C.9.5.2.1
        if (data.loc[i,"H"]<LimH1)|(data.loc[i,"H"]>LimH2):
            msg_i += "Incumple norma C.9.5.2.1 (NSR-10)\n(Lx/12 >= H >= Lx/16)\n"
        #C.10.4.1
        if data.loc[i,"Lx"]/data.loc[i,"B"]>50:
            msg_i += "Incumple norma C.10.4.1 (NSR-10)\n(Lx/B > 50)\n"
        #Altura/Base
        if (data.loc[i,"B"]<LimB1)|(data.loc[i,"B"]>LimB2):
            msg_i += "Incumple relación entre altura y base de vigas\n(H/1.4 >= B >= H/1.2)\n"
        #C.3.21.3.6.2.2
        ...
        if OK == 0:
            msg_i += "Incumple norma C.3.21.3.6.2.2 [SCWB]\n(Mnc >= 1.2Mnb)\n"
